...
import { Shield, Zap, Wifi, AlertOctagon, Terminal, Pause, Play, Network, Server, ServerOff, Eye, EyeOff } from 'lucide-react';
...
const Dashboard: React.FC = () => {
    ...
    const [hideLocal, setHideLocal] = useState(false); // Filter state
    ...
    const hideLocalRef = useRef(hideLocal);

    // Keep ref synced with state for use in interval
    useEffect(() => {
        hideLocalRef.current = hideLocal;
    }, [hideLocal]);
    ...
    }, [packets, isScanning, hideLocal]);

    // Filter packets for display
    const displayedPackets = packets.filter(p => {
        if (hideLocal) {
            // STRICT DEMO MODE: Only show our simulated actors
            return p.sourceIp.startsWith('100.') || p.sourceIp.startsWith('200.');
        }
        return true;
    });
    ...
    useEffect(() => {
        if (selectedPacket === null && displayedPackets.length > 0) {
            setSelectedPacket(displayedPackets[displayedPackets.length - 1]);
        }
    }, [packets, selectedPacket, hideLocal]);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            ...
            setPackets(prev => {
                // Find unique new packets
                const existingIds = new Set(prev.map(p => p.id));
                const uniqueNewPackets = newPackets.filter(p => !existingIds.has(p.id));

                if (uniqueNewPackets.length === 0) return prev;

                // Determine which packets count towards stats based on current filter mode
                const isDemoMode = hideLocalRef.current;
                const relevantPackets = isDemoMode
                    ? uniqueNewPackets.filter(p => p.sourceIp.startsWith('100.') || p.sourceIp.startsWith('200.'))
                    : uniqueNewPackets;
                ...
                if (relevantPackets.length > 0) {
                    ...
                    relevantPackets.forEach(p => {
                        if (p.riskLevel === RiskLevel.CRITICAL) batchThreats++;
                        batchBytes += p.length;
                        if (p.anomalyScore > maxAnomalyScore) maxAnomalyScore = p.anomalyScore;
                    });

                    // Update stats (using functional update to ensure safety)
                    setStats(prevStats => ({
                        total: prevStats.total + relevantPackets.length,
                        threats: prevStats.threats + batchThreats,
                        bytes: prevStats.bytes + batchBytes
                    }));
                    ...
                    setChartData(prevChart => {
                        ...
                        const updatedChart = [...prevChart, newEntry];
                        if (updatedChart.length > 30) updatedChart.shift();
                        return updatedChart;
                    });
                }

                const updated = [...prev, ...uniqueNewPackets];
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="hidden lg:flex gap-6 mr-6 items-center">
                        <div className="text-right border-r border-white/10 pr-6">
                            <p className="text-[10px] text-slate-500 font-mono uppercase">{hideLocal ? "Environment" : "Capture Source"}</p>
                            <p className={`text-sm font-bold flex items-center justify-end gap-1 ${hideLocal ? 'text-yellow-400' : 'text-cyan-400'}`}>
                                {hideLocal ? <Shield className="w-3 h-3" /> : <Wifi className="w-3 h-3" />}
                                {hideLocal ? "SIMULATION" : "WI-FI NETWORK"}
                            </p>
                        </div>
                        <div className="text-right">
                            <p className="text-[10px] text-slate-500 font-mono uppercase">Packets</p>
                            <p className="text-xl font-bold text-white leading-none">{stats.total.toLocaleString()}</p>
                        </div>
                        <div className="text-right">
                            <p className="text-[10px] text-slate-500 font-mono uppercase">Threats</p>
                            <p className="text-xl font-bold text-red-500 leading-none">{stats.threats.toLocaleString()}</p>
                        </div>
                    </div>

                    <div className="flex flex-col items-end mr-2">
                        <button
                            onClick={() => setHideLocal(!hideLocal)}
                            className={`flex items-center gap-2 px-3 py-2 rounded-lg border transition-all ${hideLocal ? 'border-cyan-500/30 bg-cyan-500/10 text-cyan-400 shadow-[0_0_10px_rgba(6,182,212,0.2)]' : 'border-white/10 bg-white/5 text-slate-400 hover:bg-white/10'}`}
                        >
                            {hideLocal ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                            <span className="text-xs font-mono font-bold">{hideLocal ? 'FOCUSED_VIEW' : 'UNIFIED_VIEW'}</span>
                        </button>
                        <span className="text-[10px] font-mono text-slate-500 mt-1">
                            {hideLocal ? "Showing: Demo Traffic Only" : "Showing: Real Wi-Fi + Demo Attacks"}
                        </span>
                    </div>
                    ...
                    <button
                        ...
                        title={isScanning ? "Pause Capture" : "Resume Capture"}
                    >
                        ...
                        <div className="bg-slate-900/50 border border-white/10 rounded-xl p-4 backdrop-blur-sm">
                            ...
                            <ResponsiveContainer width="100%" height="85%">
                                <LineChart data={chartData}>
                                    <XAxis dataKey="time" hide />
                                    {/* Domain set from 0.0 to 1.0 */}
                                    <YAxis hide domain={[0, 1]} />
                                    <ReferenceLine y={0.5} stroke="#f59e0b" strokeDasharray="3 3" label={{ value: "Suspicious", fill: '#f59e0b', fontSize: 10 }} />
                                    <ReferenceLine y={0.8} stroke="#ef4444" strokeDasharray="3 3" label={{ value: "Critical", fill: '#ef4444', fontSize: 10 }} />
                                    <Tooltip
                                        contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #334155' }}
